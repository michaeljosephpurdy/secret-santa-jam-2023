...
local function createRectangleBody(world, x, y, width, height, mass, image)
...
local function createCircleBody(world, x, y, radius, mass, image)
...
function Player:init(props)
	...
	if props.player_number or 1 == 1 then
		self.keys = {
			thigh_backward = "q",
			thigh_forward = "w",
			shin_backward = "e",
	...
	print(self.physics_world)
	local x = props.x or 0
	local y = props.y or 0
	self.body = createRectangleBody(self.physics_world, x + 100, y + 200, 300, 100, 5, self.body_image)
	self.thigh = createRectangleBody(self.physics_world, x + 340, y + 260, 30, 80, 5, self.thigh_image)
	self.shin = createRectangleBody(self.physics_world, x + 340, y + 360, 30, 70, 5, self.shin_image)
	self.hoof = createRectangleBody(self.physics_world, x + 340, y + 450, 30, 10, 2)
	self.head = createRectangleBody(self.physics_world, x + 380, y + 30, 100, 200, 2, self.head_image)
	self.wheel = createCircleBody(self.physics_world, x + 55, y + 210, 100, 2, self.wheel_image)
	...
	self.shin_to_hoof = love.physics.newRevoluteJoint(self.shin.body, self.hoof.body, x + 355, y + 450)
	self.hoof.fixture:setSensor(true)
...
function Player:draw()
	love.graphics.push()
	love.graphics.translate(-self.body.body:getX() + 200, 0)
